  ...
  while true; do
		  ...
		  case $sub_choice in
			  ...
			  17)
				  ...
				  send_stats "명확한 국가$country_code 的IP"
				  ;;
	...
	backup_docker() {
		...
		for c in "${TARGET_CONTAINERS[@]}"; do
			...
			if is_compose_container "$c"; then
				echo -e "${BLUE}감지됨$c 是 docker-compose 容器${NC}"
	...
	restore_docker() {
		...
		for f in "$BACKUP_DIR"/backup_type_*; do
			...
			if grep -q "compose" "$f"; then
				...
				if [[ "$running_count" -gt 0 ]]; then
					echo -e "${YELLOW}프로젝트 작성 [$project_name] 已有容器在运行，跳过还原...${NC}"
					continue
				fi

				read -e -p  "确认还原 Compose 项目 [$project_name] 경로 [$original_path] ? (y/n): " confirm
				...
				echo -e "${GREEN}프로젝트 작성 [$project_name] 已解压到: $original_path${NC}"

				cd "$original_path" || return
				docker compose down || true
				docker compose up -d
				echo -e "${GREEN}Compose 项目 [$project_name] 복원 완료!${NC}"
			fi
		done
		...
		for json in "$BACKUP_DIR"/*_inspect.json; do
			...
			if docker ps -a --format '{{.Names}}' | grep -q "^${container}$"; then
				echo -e "${YELLOW}컨테이너 [$container] 存在但未运行，删除旧容器...${NC}"
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}11.  ${color11}禅道项目管理软件                    ${gl_kjlan}12.  ${color12}Qinglong 패널 예정된 작업 관리 플랫폼"
	  ...
	  echo -e "${gl_kjlan}63.  ${color63}OpenWebUI 자체 호스팅 AI 플랫폼${gl_huang}★${gl_bai}             ${gl_kjlan}64.  ${color64}it-tools工具箱"
	  ...
	  echo -e "${gl_kjlan}69.  ${color69}SFTPGo文件传输工具                  ${gl_kjlan}70.  ${color70}AstrBot 챗봇 프레임워크"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}71.  ${color71}Navidrome私有音乐服务器             ${gl_kjlan}72.  ${color72}비트워드 비밀번호 관리자${gl_huang}★${gl_bai}"
	  echo -e "${gl_kjlan}73.  ${color73}LibreTV 개인 영화${gl_kjlan}74.  ${color74}MoonTV私有影视"
	  echo -e "${gl_kjlan}75.  ${color75}Melody音乐精灵                      ${gl_kjlan}76.  ${color76}온라인 DOS 오래된 게임"
	  ...
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}91.  ${color91}gitea 비공개 코드 저장소${gl_kjlan}92.  ${color92}FileBrowser文件管理器"
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}35.  ${gl_bai}SSH 원격 연결 도구${gl_kjlan}36.  ${gl_bai}硬盘分区管理工具"
...
echo "------------------------"
echo -e "${gl_kjlan}스크립트 공식 웹사이트:${gl_bai}https://kejilion.sh            ${gl_kjlan}GitHub地址: ${gl_bai}https://github.com/kejilion/sh${gl_bai}"
echo "------------------------"
echo ""
}
